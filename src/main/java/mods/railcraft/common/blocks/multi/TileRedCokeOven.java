...
public class TileRedCokeOven extends TileMultiBlockOven implements ISidedInventory {
...
//    private final IInventory invOutput = new InventoryMapper(this, SLOT_OUTPUT, 2, false);
    ...
    public static void placeCokeOven(World world, BlockPos pos, int creosote, ItemStack input, ItemStack output) {
        MultiBlockPattern pattern = TileRedCokeOven.patterns.get(0);
        Map<Character, IBlockState> blockMapping = new HashMap<>();
        blockMapping.put('B', RailcraftBlocks.RED_COKE_OVEN.getDefaultState());
        blockMapping.put('W', RailcraftBlocks.RED_COKE_OVEN.getDefaultState());
        TileEntity tile = pattern.placeStructure(world, pos, blockMapping);
        if (tile instanceof TileRedCokeOven) {
            TileRedCokeOven master = (TileRedCokeOven) tile;
            master.tank.setFluid(Fluids.CREOSOTE.get(creosote));
            master.inv.setInventorySlotContents(TileRedCokeOven.SLOT_INPUT, input);
            master.inv.setInventorySlotContents(TileRedCokeOven.SLOT_OUTPUT, output);
        }
    }

    @Nullable
    public TankManager getTankManager() {
        TileRedCokeOven mBlock = (TileRedCokeOven) getMasterBlock();
        if (mBlock != null)
    ...
    public IBlockState getActualState(IBlockState base) {
        return getPatternMarker() == 'W'
                ? isBurning()
                ? base.withProperty(BlockRedCokeOven.ICON, 2)
                : base.withProperty(BlockRedCokeOven.ICON, 1)
                : base.withProperty(BlockRedCokeOven.ICON, 0);
    }
