 ...
 */
public class BlockAnchorSentinel extends BlockRailcraft {

    public BlockAnchorSentinel() {
        super(Material.ROCK);
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (heldItem != null && heldItem.getItem() instanceof IToolCrowbar) {
            ...
            if (crowbar.canWhack(player, hand, heldItem, pos)) {
                ...
                if (target == null)
                    TileAnchorWorld.setTarget(ourCoord, player, getUnlocalizedName());
                else if (world.provider.getDimension() != target.getDim())
                    ChatPlugin.sendLocalizedChatFromServer(player, "gui.railcraft.anchor.pair.fail.dimension", getUnlocalizedName());
                else if (ourCoord.equals(target)) {
                    TileAnchorWorld.removeTarget(player);
                    ChatPlugin.sendLocalizedChatFromServer(player, "gui.railcraft.anchor.pair.cancel", getUnlocalizedName());
                } else {
                    if (TileAnchorWorld.isTargetLoaded(player, target, getUnlocalizedName())) {
                        TileEntity tile = WorldPlugin.getBlockTile(world, target.getPos());
                        if (tile instanceof TileAnchorWorld)
                            ((TileAnchorWorld) tile).setSentinel(player, ourCoord);
                        else if (tile != null)
                            ChatPlugin.sendLocalizedChatFromServer(player, "gui.railcraft.anchor.pair.fail.invalid", getUnlocalizedName());
                    }
                }
