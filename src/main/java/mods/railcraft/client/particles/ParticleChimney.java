...
public class ParticleChimney extends ParticleBaseSmoke {
    ...
    public ParticleChimney(World par1World, Vec3d start, Vec3d vel, float scale, EnumColor color) {
        ...
        this.particleRed = MathHelper.clamp((float) (Math.random() * 0.1f - 0.05f) + ((color.getHexColor() >> 16) & 0xFF) / 255f, 0f, 1f);
        this.particleGreen = MathHelper.clamp((float) (Math.random() * 0.1f - 0.05f) + ((color.getHexColor() >> 8) & 0xFF) / 255f, 0f, 1f);
        this.particleBlue = MathHelper.clamp((float) (Math.random() * 0.1f - 0.05f) + ((color.getHexColor()) & 0xFF) / 255f, 0f, 1f);
